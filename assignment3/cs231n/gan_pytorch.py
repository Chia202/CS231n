...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_fake = bce_loss(logits_fake, torch.zeros(N).type_as(logits_fake))
    loss_real = bce_loss(logits_real, torch.ones(N).type_as(logits_real))
...
def generator_loss(logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.ones(N).type_as(logits_fake))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = torch.mean((scores_real - torch.ones_like(scores_real).type_as(scores_real)) ** 2) / 2.
...
def ls_generator_loss(scores_fake):
    ...
    # pass
    loss = torch.mean((scores_fake - torch.ones_like(scores_fake).type_as(scores_fake)) ** 2) / 2.
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250,
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            print(f'x = {x.shape}, fake_images = {fake_images.shape}')

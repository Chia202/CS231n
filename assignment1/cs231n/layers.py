...
def svm_loss(x, y):
    ...
    margins[margins > 0] = 1
    margins[np.arange(N), y] -= margins.sum(axis=1)
    dx = margins / N
...
def softmax_loss(x, y):
    ...
    # expx = np.exp(x)
    # sum_exp_scores = np.sum(expx, axis=1)
    # loss = np.sum(-correct_class_score + np.log(sum_exp_scores)) / N
    # dx = expx / sum_exp_scores[:, np.newaxis]
    # dx[np.arange(N), y] -= 1.
    # dx /= N
